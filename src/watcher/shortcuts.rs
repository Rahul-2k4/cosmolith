...
pub fn start_shortcuts_watcher(
    ...
    let config = Config::new(SHORTCUTS_NAMESPACE, VERSION)
        .map_err(|source| Error::watcher_init("shortcuts", source))?;
    ...
    if let Ok(sender) = tx.lock() {
        for (binding, action) in initial_shortcuts.iter() {
            sender
                .send(Event::Shortcut(ShortcutEvent::Add {
                    binding: binding.clone(),
                    shortcut: action.clone().into(),
                }))
                .map_err(|source| Error::event_send("shortcuts", source))?;
        }
    }
                ...
                if let Ok(sender) = tx.lock() {
                    if let Ok(mut state) = state.lock() {
                        ...
                        for (binding, action) in old_shortcuts.iter() {
                            if !new_shortcuts.contains_key(binding)
                                || new_shortcuts.get(binding) != Some(action)
                            {
                        ...
                        for (binding, action) in new_shortcuts.iter() {
                            if !old_shortcuts.contains_key(binding)
                                || old_shortcuts.get(binding) != Some(action)
                            {
